...
class HotNewsToolAdapter:
    ...
    async def get_hot_news_formatted(
        self, platform: str, limit: int = 20, format_type: str = "summary"
    ) -> str:
        """获取格式化的热点新闻"""
        try:
...
async def create_hot_news_client(
    ...
    config = HotNewsClientConfig(
        server_path=server_path, server_url=server_url, transport=transport
    )
    return HotNewsMCPClient(config)


async def quick_get_news(
    platform: str, limit: int = 20, server_path: Optional[str] = None
) -> Optional[PlatformNews]:
    """快速获取新闻的便捷函数"""
