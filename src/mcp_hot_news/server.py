...
class HotNewsProvider:
    ...
    def __init__(self):
        ...
        self.platforms = {
            ...
            "kuaishou": {"name": "快手热点", "api_path": "douyinHot", "vvhan": True},  # 使用抖音API
            "netease": {"name": "网易新闻", "api_path": "baiduRY", "vvhan": True},    # 使用百度API
            "thepaper": {"name": "澎湃新闻", "api_path": "baiduRY", "vvhan": True},   # 使用百度API
            "xueqiu": {"name": "雪球", "api_path": "hupu", "vvhan": True}             # 使用虎扑API
        }
    
    async def get_platform_news(self, platform: str, limit: int = 20) -> Optional[PlatformNews]:
        """获取指定平台的热点新闻"""
                    ...
                    if response.status_code == 200:
                        ...
                        if data.get("success", False) and "data" in data:
                            ...
                            if isinstance(raw_data, dict) and "list" in raw_data:
                                ...
                                logger.warning(f"vvhan API返回意外格式 - 平台: {platform_name}")
                            ...
                            for i, item in enumerate(items):
                                if isinstance(item, dict):
                                    news_item = NewsItem(
                                        ...
                                        hot_value=item.get("heat", 0) or item.get("hot", 0),
                                        rank=i + 1,
                            ...
                            logger.warning(f"vvhan API返回失败 - 平台: {platform_name}, 响应: {data}")
                            return await self._get_mock_news(platform, limit)
                    else:
    ...
    async def _get_mock_news(self, platform: str, limit: int) -> PlatformNews:
        ...
        # 根据平台生成不同类型的模拟数据
        base_titles = {
            "zhihu": ["知乎热门话题", "深度思考问题", "专业领域讨论", "生活经验分享", "科技前沿探讨"],
            "weibo": ["微博热搜话题", "娱乐明星动态", "社会热点事件", "网络流行话题", "突发新闻"],
            "baidu": ["百度热搜关键词", "搜索热门词汇", "网民关注焦点", "热门搜索词", "流行搜索"],
            "bilibili": ["B站热门视频", "UP主创作", "动漫番剧", "游戏解说", "知识科普"],
            "douyin": ["抖音热门话题", "短视频话题", "创意挑战", "音乐热门", "生活记录"],
...
def main():
    ...
    parser.add_argument(
        "--host", 
        default="0.0.0.0",
        help="HTTP服务器主机 (默认: 0.0.0.0)"
    )
    parser.add_argument(
        "--port", 
        type=int, 
        default=8001,
        help="HTTP服务器端口 (默认: 8001)"
    )
    parser.add_argument(
        "--log-level",
